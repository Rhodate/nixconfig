{writeShellScriptBin, system, inputs, ...}: let
  swww = inputs.swww.packages.${system}.swww;
in writeShellScriptBin "swww-randomise" ''
...
DISPLAY_LIST=$(${swww}/bin/swww query | grep -Po "^[^:]+")
    ...
    find "$1" -type f \
        ...
        | while read -r img; do
            ...
            for disp in $DISPLAY_LIST; do
                ...
                ${swww}/bin/swww img --resize=$RESIZE_TYPE --outputs $disp $img
