...
{
  config =
    mkIf (config.swarm.server.k3s.enable && (length config.swarm.server.k3s.zfsStorageDisks) > 0)
      {
        services.k3s = {
          extraFlags = mkAfter [ "--node-label openebs.io/engine=mayastor" ];
          manifests = listToAttrs (
            map (zfsStorageDisk: rec {
              name = "diskpool-${config.networking.hostName}-${
                replaceStrings [ "/" "." ] [ "-" "-" ] zfsStorageDisk
              }";
              value = {
                content = {
